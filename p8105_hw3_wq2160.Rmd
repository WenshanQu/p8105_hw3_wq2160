...
There are `names(accel_df)` 6 variables after tidying the data. We create a new variable "day_type" according to "day" values, and we pivoted longer this dataset. Finally, there are `nrow(accel_df)` rows and `ncol(accel_df)` columns in this dataset. There are `nrow(distinct(accel_df, week))` weeks, and in each week, there are `nrow(distinct(accel_df, day))` days. Also, for each day, there are 1440 observations.
